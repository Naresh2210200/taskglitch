...
export function computeROI(revenue: number, timeTaken: number): number | null {
  // Safe ROI calculation with validation
  // Return null for invalid inputs or division by zero
  if (!Number.isFinite(revenue) || !Number.isFinite(timeTaken)) {
    return null;
  }
  if (timeTaken <= 0) {
    return null; // Division by zero or negative time
  }
  const roi = revenue / timeTaken;
  // Return null if result is not finite (shouldn't happen with above checks, but safety first)
...
export function sortTasks(tasks: ReadonlyArray<DerivedTask>): DerivedTask[] {
  return [...tasks].sort((a, b) => {
    ...
    // Stable tie-breaker: use task title (alphabetical) for deterministic ordering
